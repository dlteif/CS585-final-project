...
deblurred_wiener = sorted(x for x in glob('../wiener_deconvolution/results/*'))

dl_metric = []
norm_metric_list = []
wiener_metric_list = []

for imgs in zip(ground_truth_imgs,deblurred_imgs, deblurred_normed, deblurred_wiener):
    gt_img_path, deblurred_img_path, norm_img_path, wiener_path = imgs
    gt = cv2.imread(gt_img_path)
    db = cv2.imread(deblurred_img_path)
    norm = cv2.imread(norm_img_path)
    wiener = cv2.imread(wiener_path)
    ...
    gray_wiener = cv2.cvtColor(wiener, cv2.COLOR_BGR2GRAY)
    ...
    wiener_metric = cv2.Laplacian(gray_wiener, cv2.CV_64F).var()
    dl_metric.append(db_metric - gt_metric)
    norm_metric_list.append(norm_metric - gt_metric)
    wiener_metric_list.append(wiener_metric - gt_metric)
...
wiener_metric_avg = sum(wiener_metric_list)/len(wiener_metric_list)



print('DL Average Improvement: {}'.format(dl_metric_avg))
print('Norm Average Improvement: {}'.format(norm_metric_avg))
print('Wiener Average Improvement: {}'.format(wiener_metric_avg))
